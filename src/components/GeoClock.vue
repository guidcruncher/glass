...
const props = defineProps({
  ...
  twilightLevel: {
    ...
    validator: (value) =>
      ["none", "civil", "nautical", "astronomical"].includes(
        value.toLowerCase(),
      ),
  },
});
...
function updateGeoClock() {
  ...
  const solarDeclination =
    -23.44 *
    Math.cos(((dayOfYear + 10) / 365.25) * 2 * Math.PI) *
    (180 / Math.PI);
  const hoursUTC =
  ...
  const antipodeLong =
    subsolarLong > 0 ? subsolarLong - 180 : subsolarLong + 180;
  ...
  const solarTerminatorBoundary = d3
    .geoCircle()
    .center([antipodeLong, antipodeLat])
    .radius(90)();
